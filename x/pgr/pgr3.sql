  ...
  begin
    ...
    */
    for rot1 in (
      select *
      from routes
      where route_name = '寺９１'
    ) loop
      raise notice '%', rot1.route_name;
      for ptn1 in (
        select trip_patterns.*
        from trip_patterns
        where 
          feed_id = rot1.feed_id and
          route_id = rot1.route_id and
          pattern_id in (390, 391, 392)
      ) loop
        ...
        */
        ...
        update map.edges set indivmultiplier = 0.1 where id in (select id from reses);
      ...
      update
        ...
        from map.results
        where
          results.feed_id = rot1.feed_id and
          results.route_id = rot1.route_id and
          -- ((map.edges.pattern_id = pptn and type in ('外郭', '分割')) or 
          -- (map.edges.pattern_id = ptn1.pattern_id and type = '路線')) and
          st_dwithin(
            map.results.geom,
