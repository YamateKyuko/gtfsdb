...
insert into temps
	select s.feed_id, stop_id, stop_name, st_point(stop_lon, stop_lat) as stop_geom, false, null, sum(cnt) as cnt
		from stops as s
    left join stop_patterns as p using(feed_id, stop_id, stop_name)
    left join daytype_cnt as c on p.pattern_id = c.pattern_id and daytype = '平'
    group by s.feed_id, stop_id, stop_name, stop_lon, stop_lat
		order by stop_id;
...
insert 
  ...
  select distinct on (station_id) * from tbl;
